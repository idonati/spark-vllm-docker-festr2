...
- ✅ **CUDA graph capture (decode path)** — fixed via a CLASS-LEVEL shared scratch buffer (`_tq_mid_o_scratch`, `_tq_lse_scratch`) on `TritonAttentionDiffKVImpl`, allocated ONCE on first decode call at fixed capacity 512 × Hq × 8 × (Hv+1) fp32 = 8 MB total (shared across all 70 TQ-K8V4 layers since they have uniform Hq, Hv). The launcher accepts optional pre-allocated scratch via two new args; falls back to per-call allocation when omitted (used by unit tests and the prefill fallback). Hard-fails if a workload exceeds 512 q_max, with a clear error pointing to the constant to bump. CUDA graph capture sees a stable data pointer across captures and replays. Prefill path (max_query_len > 1) bypasses scratch entirely — it runs eager via the dequant fallback. **First C33 launch will still use `--enforce-eager`** to derisk the validation run; graphs can be re-enabled in a follow-up cycle once eager numerics check out.
- ✅ **MTP draft-head path** — `patch_mimo_v2_mtp_cache_config.py` threads `cache_config` through `MiMoV2MultiTokenPredictor` → `_MiMoV2MTPLayers` → `MiMoV2MTPLayer` → `MiMoV2Attention(... cache_config=cache_config, ...)`. 5 inserts; idempotent; AST-validated against the on-disk source. Wired into `launch-cluster.sh` as Component 1b under `ENABLE_TQ_K8V4=1`.
- ⏸️ **End-to-end engine launch + numerical validation against C32** — BLOCKED on a Ray-side hang during executor init that is independent of TQ-K8V4 code. 3 launch attempts (2026-05-16): model + drafter loads cleanly on all 8 ranks (TP + MTP), but EngineCore deadlocks at `ray.get(init_worker_refs)` in `ray_executor_v2.py:398` despite all worker actors being idle in Ray's `main_loop` (verified via py-spy on every node). No tracebacks. No errors in Ray logs or per-worker core logs. Pattern reproduces across launches with HF_HUB_OFFLINE / TRANSFORMERS_OFFLINE propagated, and independent of AO6 NVMe slowness (issue happens AFTER AO6 finishes its load). The TQ-K8V4 path is never exercised — workers all return from `initialize_worker` BEFORE the post-init engine setup phase where TQ would matter. This is a deployment-side Ray executor v2 issue to debug separately. Operational gotcha discovered while debugging: recipe `env:` block only exports vars in the foreground shell of `vllm serve` on the head — Ray actor processes do NOT inherit; must pass via `-e VAR=VAL` to `run-recipe.py` for docker `-e` propagation.

M1 code is complete and shipped (commits `6c25595`, `25d06ea`, `dd87344`, `7f84313`, plus MTP plumbing). Engine launch validation deferred pending Ray hang debug. Numerical-correctness gate can be re-attempted once the Ray hang is understood; my smoke-test evidence is unit-level proof of correctness for the TQ store/decode/prefill/dequant kernels.
